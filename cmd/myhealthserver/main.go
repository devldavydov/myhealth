...
import (
	...
	_log "github.com/devldavydov/myhealth/internal/common/log"
...
func run() error {
	...
	logger.Info("Started MyHealth Server", zap.String("buildDate", buildDate), zap.String("buildCommit", buildCommit))

	serviceSettings, err := ServiceSettingsAdapt(config)
	if err != nil {
		return fmt.Errorf("failed to create server service settings: %w", err)
	}
